...
- Implementation subject: `elmakus/pi-unraid@1470e9340e494d7e8ea05a176531bec66df66f6e` plus uncommitted spawn-witness repair below (Main commits; exact new SHA assigned at commit; priors `f97f69fd36047521d6e0346d0db78291eafd1f13`, `f2fcf37b4932fc33a8b206a42cf2341b97859b8f`, `f914dcf355528691dd36913cd8c60eb8a1dbfddf`).
- Exact-SHA CI: [run 36266005842](https://github.com/elmakus/pi-unraid/actions/runs/36266005842), SUCCESS on `f2fcf37` (direct-Pi/workspace legs, HEAD-only recovery); [run 36267353405](https://github.com/elmakus/pi-unraid/actions/runs/36267353405), RED on `f914dcf` (only the spawn probe failed; suite, readback, build, strengthened recovery flow, and production refusal all passed, and no failure report was captured); [run 36268682202](https://github.com/elmakus/pi-unraid/actions/runs/36268682202), RED on `1470e93` (only the spawn probe failed; everything else GREEN, with a full secret-safe diagnostics report captured). A re-run on the new SHA is required for the spawn leg.
...
- Contract suite: 16 suites / 284 tests GREEN locally (15 pre-existing suites 243 tests plus `test_paseo_rpc_workspace_contract.py` 41 tests incl. ownership/visibility, recovery, and spawn/diagnostics/witness regressions); 278-test suite GREEN in CI run 36268682202; re-run on the new SHA pending.
- Static readback (local and CI run 36268682202): verdict `technical_green_ha_outstanding`, zero violations, `full_green_claimed: false`.
- Disposable Docker flow: GREEN in CI run 36268682202 for direct-Pi, strengthened content-verified recovery, and workspace legs; only the Paseo-driven spawn witness is RED and repaired pending re-run. No Docker in this workstation; live execution happens only on the hosted runner.
...
| --- | --- | --- | --- |
| Exact frozen candidate Pi RPC on-demand | Direct `pi --mode rpc --no-session` probes x2 (child mechanism) plus Paseo-driven `run --provider pi` spawn probe (orchestration) | Policy checks GREEN | Direct leg GREEN in all runs; Paseo-driven leg RED in runs 36267353405/36268682202 with diagnosed witness bug, repair pending re-run (not inferred from direct leg) |
...
## RED run 36268682202 and witness repair (uncommitted; re-run pending)

- Run: https://github.com/elmakus/pi-unraid/actions/runs/36268682202 on `1470e9340e494d7e8ea05a176531bec66df66f6e`. Suite (278), readback, build, recovery flow, and production refusal GREEN; spawn probe RED with a full diagnostics report that diagnosed the witness bug instead of a product failure.
- Observed proof of a real spawn: `ps_full` showed PID 219 (PPID 48, command `pi`) as a live direct child of PID 48 `Paseo Daemon` (confirmed as the daemon worker by `paseo status` `workerPid: 48`, with the Pi provider `available: true`); `paseo ls --json` showed our agent `d6057fcc-ff7f-462b-a0aa-ed51e740a4a3` with provider `pi/unknown/unknown` (status `error` after ~1 minute, cause unclassified because the table agent ID went unparsed so no timeline was fetched). Container PIDs vary per run and are not hardcoded anywhere.
- Root cause: the old predicate grepped for literal `pi --mode rpc`, but the daemon's pi child presents bare `pi` (argv/title normalization), so a live Paseo-spawned Pi was missed for the whole 90s window. Secondary gap: `parse_agent_id` handled only JSON, not the CLI table shape, so agent logs were never fetched.
- Repair in this change: the witness matches exact comm/argv-basename `pi` from full `ps -eo pid,ppid,comm,args` output and binds the child to this container's actual daemon worker pid parsed from `paseo status` (strict `worker_pid` match required by CI; comm fallback only produces a diagnosable report, never CI GREEN); the agent ID parses from the CLI table and binds against `paseo ls --json` provider `pi`; `/proc/<pid>/cmdline` and `exe` snapshots are recorded best-effort; the agent timeline classifies turn errors as `model_auth` (HA-owned), `other`, or `unknown` without affecting the spawn verdict, since the Card needs launch proof, not turn success. RPC provider semantics stay a distinct two-part proof (frozen v0.9.2 process-backed provider docs plus the GREEN direct `pi --mode rpc --no-session` smokes), recorded in the report without inferring a model turn.
- Seven regression tests use the observed `ps_full` shape: accepted daemon-child binding, wrong-parent rejection, false-positive avoidance (`pip`, similar basenames, flag text), workerPid parse, table/JSON ID parse, pi-agent binding incl. mismatches, and error classification.
- What remains to re-prove in CI: the repaired witness must observe a daemon-parented `pi` child bound to a pi-provider agent on the exact new SHA; first-auth turn failure, if shown by the timeline, stays a classified HA-owned note, not a spawn failure.

## Residual risks

- Direct-Pi RPC handling follows the M02 smoke contract and is proven GREEN on this exact candidate/image pair; the repaired spawn witness executes for the first time on the next CI run.
- If the agent timeline shows the `error` status comes from missing model auth, that turn failure is classified `model_auth` and owned by the already-planned M07-T02 staged interactive proof (secret supply plus first provider/account interaction), reusing this probe's daemon/CLI mechanics with credentials present.
