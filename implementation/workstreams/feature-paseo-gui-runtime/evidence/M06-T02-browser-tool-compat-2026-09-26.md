...
- Implementation state: base `1902848ce73049916e88a830f67e282c081d6567` pushed to origin plus uncommitted R3 repair files. Worker harness forbids git writes, so commit/push and the exact-SHA CI re-trigger are Main-owned next steps.
- Exact-SHA CI run [36273187258](https://github.com/elmakus/pi-unraid/actions/runs/36273187258) on `868915c`: FAILURE. Predecessor bindings, 317-test suite, static readback, and frozen-candidate build/test passed; `Disposable browser/tool/extension compatibility flow` failed at the headed/Xvfb leg; refusal step skipped; untouched proof and artifact upload passed. Seed image `sha256:4d132c13db4f9f298b3612b68b7e7d7f15170d7265f999b65a02636edb2aee87` tag `pi-unraid:paseo-b4e0c1e7c276`. No live Docker leg ran in this workstation (no Docker daemon); live execution happens only on the hosted runner.
- Root cause (RED run): the headed leg ran `launchPersistentContext` under `xvfb-run` as numeric `--user 99:100`, which has no passwd entry and no writable HOME in the container; Chromium headful crashed at launch (`browserType.launchPersistentContext: Target page, context or browser has been closed`) while headless tolerated the same context. The M01-T03 headed precedent ran as the image default user.
- Repair R1 (committed as `f22f08f`): browser legs run as the image default user via `browser_flags()` (M01-headed-proven context, shm kept); fixture profile/downloads chmodded `0777` for default-user writes; browser probes keep full secret-scanned output on failure; failure reports now include `phases_completed`. No assertion weakened, no phase excluded, no candidate/image input changed.
- Exact-SHA CI run [36273641228](https://github.com/elmakus/pi-unraid/actions/runs/36273641228) on `f22f08f`: FAILURE. Predecessor bindings, 325-test suite, static readback, and frozen-candidate build/test passed; flow failed in fixture setup with `[Errno 1] Operation not permitted: '/tmp/pi-unraid-m06-t02.428_vvmc/profile'` before any phase ran. Seed image `sha256:89b6ffd4fcc01ec381cc108ba908cfde79f300d853bf6ce3d007b072ecdc38a5` tag `pi-unraid:paseo-b4e0c1e7c276`. The R1 headed-user fix is therefore still unproven live.
- Root cause (R2): R1 called `prepare_browser_dirs` (host chmod) after `chown_fixture` transferred fixture ownership to 99:100, so the host runner could no longer chmod. Ordering defect in the repair, not in Card acceptance.
- Repair R2 (committed as `1902848`): chmod before chown (mode bits survive the recursive chown), and fixture setup moved inside the phases-carrying failure-report block. No assertion weakened, no phase excluded, no candidate/image input changed.
- Exact-SHA CI run [36274045464](https://github.com/elmakus/pi-unraid/actions/runs/36274045464) on `1902848`: FAILURE. Predecessor bindings, 326-test suite, static readback, and frozen-candidate build/test passed; flow completed `fixture`, `image_labels`, `browser_headless`, `browser_headed_xvfb`, then failed at `profile_isolation` with `automation profile Preferences missing; persistent profile unproven`. Seed image `sha256:eaeb6bf025e6cd1607892d0e41f7f056e4711052badd2af4e8b5b7e6375977c4` tag `pi-unraid:paseo-b4e0c1e7c276`. R1 headed-user fix is now proven live (both browser legs GREEN).
- Root cause (R3): wrong persistence-marker path in the harness. Chromium stores the default profile under the user data dir, so the marker is `Default/Preferences`, not top-level `Preferences`; the profile directory was populated (entries assert passed) but the file assert pointed at the wrong path. Assertion-path defect, not a product or acceptance defect.
- Repair R3 (uncommitted, in worktree): persistence marker corrected to `Default/Preferences` with non-empty check, recorded as `preferences` detail, CI workflow asserts the new detail. No assertion weakened, no phase excluded, no candidate/image input changed. Re-run CI is expected to reach the dev/gh/docker/extension legs.
...
- Contract suite: 17 suites / 327 tests GREEN locally (16 pre-existing suites 284 tests plus `test_paseo_browser_tool_compat_contract.py` 43 tests incl. pin/readback/guard/profile/download/wishlist-deferral/workflow plus browser-user/diagnostics/failure-report/fixture-ordering/profile-layout regressions).
...
- Claimed now: implementation + static acceptance GREEN in worktree with R3 profile-layout repair uncommitted; live flow RED on runs 36273187258, 36273641228, 36274045464 (browser legs now live-GREEN through run 36274045464); M06-T02 stays `in_progress` at Board rev 101. No GREEN live-flow claim.
- Next: Main commits the R3 repair (harness + tests + workflow + evidence), pushes `feat/paseo-gui-runtime` without force, records the new exact-SHA CI run of `paseo-browser-tool-compat.yml`; a follow-up worker turn then binds the CI result (or repairs any further RED within Card scope) before any result freeze.
