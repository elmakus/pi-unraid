...
P3 preserves the previously GREEN Paseo/Pi strategy and P2 host-control acceptance timing while applying Definition R2's local-first build/rollback authority. The current deployment path no longer depends on a private project GHCR, registry-backed recovery cache or a self-hosted GitHub runner. The required path is a dedicated persistent Tower Buildx/BuildKit builder/cache plus bounded local immutable known-good child-image retention for staged promotion and rollback. It is governed by requirements/PASEO_GUI_RUNTIME.md and ADR-PGR-001 through ADR-PGR-004. Earlier Definition Research remains factual support for implementation choices but does not replace those authorities.
...
The existing standalone Phase-1 Pi image, compose shape and scripts are a migration baseline only. They may be reused where their behavior still satisfies R2, but their Node-base image, /home/pi identity, sudo-oriented bootstrap and standalone-Pi lifecycle must not be carried forward by inertia.
...
## 2.1 P3 change boundary

P3 retains P2's host-control acceptance timing: PGR-REQ-045–054 remain unchanged, M03-T01 may complete from implementation plus exact live API/schema/version readback, fail-closed behavior and non-secret CI/static evidence, and M06-T04 still owns the deferred authenticated least-privilege readback plus reversible mutation/restoration smoke before production cutover. P3 additionally changes the M05/M07 build-delivery strategy to the promoted local-first authority. Private project GHCR publication, registry-backed cache and a self-hosted GitHub runner are not prerequisites. M05-T02A's existing GREEN local Buildx/cache foundation remains accepted; not-yet-materialized downstream work must prove persistent Tower builder/cache survival, bounded local immutable-image retention, safe prune behavior and rollback. Complete loss of local image/cache state is recovered by rebuilding from canonical Git plus the frozen candidate/provenance.
...
- M03-T01 GraphQL-primary control foundation: define the dedicated Unraid API credential contract, least-practical role/permission profile compatible with accepted administration, structured client/readback, bounded ordinary mutation surface, exact live Unraid/API version/schema readback, fail-closed unauthenticated behavior and CI/static safety proof. Credential-backed authenticated readback and live mutation are intentionally deferred to M06-T04, where the complete integrated runtime exists.
...
Exit: the GraphQL-primary client, permission contract, safety boundary and host-control implementation are reviewable and mechanically testable; credential-backed live mutation proof is an explicit downstream acceptance obligation in M06-T04 rather than a prerequisite for completing the M03 implementation foundation.
...
- M05-T02 Tower-local Buildx/cache/retention path: preserve the accepted M05-T02A local Buildx foundation, prove a dedicated persistent Tower Buildx/BuildKit builder/cache survives ordinary runtime replacement, retain a bounded set of local immutable known-good child images for staged promotion/rollback, prune only after successful work without deleting the active rollback set, and keep cache-friendly Dockerfile stages/cache mounts/rebase techniques where measured useful. Private project GHCR, registry cache and a self-hosted GitHub runner are excluded from the required current path.
...
- M06-T04 Host-control integration smoke: using a dedicated least-privilege credential, perform authenticated structured INFO/DOCKER readback and at least one safe reversible ordinary GraphQL container mutation with exact pre-state/post-state/restoration readback; prove the selected bounded permission profile is sufficient without a broader role; then perform the forced bounded SSH-fallback exercise for a safe gap/recovery case, host doctor checks and return-to-GraphQL behavior. Raw credentials remain outside Git/evidence.
...
- M07-T01 Read-only Tower readiness: refresh Unraid version/API state, appdata/workspace ownership, current Paseo absence/presence, backup coverage, secrets, persistent Tower builder/cache readiness, local immutable-image/rollback-anchor readiness and legacy Pi state. Private project GHCR/runner readiness is not part of the required current deployment path. Any newly required high-impact operation stops for user authority before mutation.
...
- M06 consumes GREEN implementation results from M01–M05. For host control specifically, M03 GREEN means the implementation/safety foundation is accepted; M06-T04 owns the deferred credential-backed live GraphQL mutation proof before integrated acceptance can become GREEN.
...
The milestone coverage is intentionally overlapping because acceptance validates cross-cutting behavior, but every R2 requirement has a primary home:
...
- PGR-REQ-045–054: M03 implementation/safety foundation primary; credential-backed GraphQL mutation and fallback integration acceptance is mandatory in M06-T04, with M07 retaining production readback.
...
No requirement is intentionally deferred except the integrations that R2 itself declares separate later scope.
...
- M03: GraphQL/SSH/gate/host-doctor implementation evidence, live Unraid/API version/schema readback, fail-closed endpoint behavior, permission-profile definition and CI/static safety proof; no credential-backed live mutation is required at M03-T01.
...
- M06: integrated Pi RPC/Relay/browser/GitHub/extensions/host-control matrix, including the deferred authenticated least-privilege GraphQL readback plus reversible mutation/restoration proof in M06-T04.
...
- Secret leakage: native secret mechanisms, sensitive HOME handling, bounded fingerprints only in doctor/evidence. M03-T01 does not require interactive secret handoff solely to prove an isolated mutation; the credential-bearing live proof is consolidated into M06-T04 integrated acceptance.
- Capability-policy duplication: inventory stores availability only; OR policy remains external.
- Build regressions: measured phase timing, persistent Tower-local cache, bounded local known-good image retention and treated-as-defect unnecessary rebuilds; total local cache/image loss is recovered by deterministic rebuild from canonical Git plus the frozen candidate/provenance.
...
P3 is complete when this exact plan has:
...
After freeze, Premium B requires a fresh independent best-available review context. The planning context that authored P3 must not perform that review.
