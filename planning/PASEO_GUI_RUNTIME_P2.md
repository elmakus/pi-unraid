# Paseo/Pi Runtime on Unraid — Strategic Plan P2
...
P2 preserves the GREEN Paseo/Pi Definition and the P1 architecture while changing one acceptance-timing decision: credential-backed live GraphQL mutation proof moves from M03-T01 implementation acceptance to the already-planned M06-T04 integrated host-control smoke. It is governed by requirements/PASEO_GUI_RUNTIME.md and ADR-PGR-001 through ADR-PGR-004. Definition Research R1 is factual support for implementation choices but does not replace those authorities.
...
## 2.1 P2 change boundary

P2 does not waive PGR-REQ-045–054 and does not weaken the least-privilege or pre/post-readback requirements. It changes only **when** credential-bearing live mutation evidence is mandatory. M03-T01 may complete from implementation, exact live API/schema/version readback, fail-closed behavior and non-secret CI/static evidence. M06-T04 cannot complete without the deferred authenticated least-privilege readback and reversible mutation/restoration smoke. If that M06-T04 smoke fails, integrated acceptance remains RED/blocked and production cutover cannot proceed.
...
P2 is complete when this exact plan has:
...
After freeze, Premium B requires a fresh independent best-available review context. The planning context that authored P2 must not perform that review.
