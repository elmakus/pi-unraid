...
Planner audit: `planning/audits/R2.md`
Independent review record: `planning/reviews/R2.md`
Date: `2026-09-23`

## 1. Revision purpose, authority and verified baseline

R2 reorganizes the remaining Phase 1 work after the accepted Definition changed from direct Pi ChatGPT/Codex OAuth to mandatory Codex-LB-backed access from the first real model interaction.

This revision does **not** reopen completed implementation. M01 and M02 remain accepted historical checkpoints with their existing evidence. R2 supersedes only the not-yet-accepted future R1/M03 strategy and the superseded `M03-T01` execution attempt. No production deployment occurred under that superseded Card.

Authoritative inputs:

- `requirements/PI_UNRAID_BOOTSTRAP.md`, revision R2, approved; all 24 requirements and all 21 acceptance-level outcomes remain binding.
...
R1's independent GREEN review remains valid evidence that the original plan was coherent for Definition R1. It does not approve the R2 Codex-LB execution strategy.
...
- No production `pi-unraid` container was deployed before Definition R2.
...
Codex-LB is in scope only as the already-selected external ChatGPT/Codex access dependency required by Definition R2.
...
## 3. R2 execution strategy

R2 keeps the M01 → M02 → M03 milestone chain. M01/M02 are historical completed checkpoints. Only M03 remains executable.
...
The existing Tower Codex-LB instance is the intended dependency, but the R2 research snapshot is not permanent freshness proof. Execution Prep must refresh its fork/image/runtime state before deciding whether it is already acceptable.
...
- an authorized Docker/host restart proves expected restart-policy recovery;
...
R2 does not create replacement M01 packages or rerun M01 merely because model access moved to Codex-LB. M03 performs only the live integrated re-verification required by Definition R2.
...
R2 does not reopen M02. M03 reuses its accepted operational mechanisms against the actual production deployment and records any target-specific evidence.
...
- explicit permitted window before Docker-wide/host restart or similarly disruptive verification;
...
- **M03-W4 — Production recovery, restart and final handoff.** Exercise normal update, safe Pi-runtime fallback, actual retained-image rollback, planned-stop/session resume, bounded logs, authorized Docker/host restart, Codex-LB routing/dependency separation and workstation independence; consolidate acceptance and operator recovery evidence.

**Checkpoint:** every Definition R2 acceptance outcome is GREEN with exact evidence or remains visibly incomplete/blocked. No mock-only substitute counts for the real Codex-LB model interaction, production restart/recreation, real rollback or authorized host/Docker restart. No acceptance evidence may expose the Pi client credential or upstream OAuth/token material.
...
**JIT trigger:** accepted M02 artifacts + refreshed M03 target/dependency readiness make concrete network path, secret injection, provider configuration, dependency reconciliation status, UID/GID, source fast-forward, test repository/worktree, live credential action and restart window knowable. Execution Prep then creates new M03 Cards. The superseded R1 `M03-T01` is historical only and must not be revived or edited into a new authority surface.

## 5. Complete requirement coverage

Every authoritative R2 requirement maps to an owner milestone and a future execution path. M01/M02 entries below are historical ownership; M03 re-verifies the live integrated target where required.
...
| PIB-REQ-013 | M01 → M03-W4 | Europe/Zurich and `unless-stopped` behavior, including authorized restart proof. |
...
`A01`–`A21` below are local labels for the numbered Definition R2 acceptance outcomes, not additional requirements.
...
| A02 (2) | M01 → M03-W4 | Running service returns after authorized host/Docker restart according to restart policy. |
...
The R2 integrated matrix must cover at least:
...
| Authorized Docker/host restart | Pi restart policy recovers as specified; Codex-LB/workstation impact is observed honestly; no claim of project ownership over those services. |
...
- **Disruptive restart gate:** Docker-wide/host restart or another action that can interrupt unrelated services.

A single explicit authorization may cover multiple concrete operations when its scope is clear; do not manufacture per-command approval friction.
...
After R2 approval, Execution Prep must:

- refresh Tower and Codex-LB facts that can drift;
- reconcile the superseded R1 `M03-T01` as historical only;
- materialize concrete Cards covering M03-W1..W4 before implementation;
...
This R2 revision is materially different from R1 and the user explicitly requires independent Plan Review, so review classification is **REQUIRED**.

The planner's own audit must be GREEN before freeze. The exact R2 plan subject is then frozen as one immutable Git blob, `planning/reviews/R2.md` becomes `pending`, and the selected workstream manifest points `routing.plan_review` to that record.

This authoring session must stop at that fresh independent Plan Review boundary. It must not issue its own independent verdict, approve R2, enter Execution Prep, create new M03 Task Cards or perform any Tower/Codex-LB live-write.
