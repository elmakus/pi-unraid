...
`config/unraid-host-control/host-safety-policy.json` classifies the existing GraphQL one-container actions as ordinary bounded mutations with transport-internal pre-readback. It classifies the following as user-gated:
...
The generic SSH administrative path is intentionally not an ordinary shell escape. It requires three external artifacts bound to the exact SHA-256 scope of the command argv: GREEN pre-mutation readback, a private GREEN `external_user_authorization` record, and a rollback anchor when the command declares rollback applicable. This repository provides no command that manufactures the user-authorization record.
